...
암호화는 다음의 과정을 거친다.  
1. 하나의 Plaintext Block을 반으로 나눈다.(L<sub>0</sub>, R<sub>0</sub>)  
2. round를 진행하는데, 각 round 마다 (i = 1, 2, ..., n)
...
위의 디자인이 Feistel Cipher로 F(Round Function)의 차이에 알고리즘의 좋고 나쁨이 결정된다.
...
(E : 암호화, D : 복호화, P : Plaintext, C : Ciphertext)  

위와 같이 암호화, 복호화를 진행하는데, 의문점이 생긴다.  
1. 왜 Key를 2개만 사용할까?
    - K<sub>1</sub> + K<sub>2</sub> = 112bits로 충분히 많기 때문에 K<sub>3</sub>를 사용할 필요성을 느끼지 못해서이다.  
    - key는 전수조사를 할 수 없는 정도이어야한다. 하지만 너무 길면 관리가 어려워진다.
2. 암호화를 3번 하면 되는데 왜 암호화와 복호화를 섞어서 할까?
    - 호환성을 위해서 이다. 위의 그림에서 K<sub>2</sub> 대신 K<sub>1</sub>을 이용하면 DES처럼 돌아간다.
3. 왜 Triple일까? Double DES는?  
    - 전수조사를 하지 않아도 공격 방법이 발견되었기 때문이다.

Triple DES는 DES를 대신해 사용되었는데, 
이후 더 좋은 알고리즘인 AES의 등장으로 잘 사용하지 않는다.  
***
### AES  
...
Key addition는 섞은 byte들을 subke와 XOR한다.  
<img src="/assets/images/Security_AESaddKey.PNG" width=500 height= 400>  

위의 과정을 거쳐 다음 Round로 거치고 최종적인 ciphertext가 나온다.    
